...
def _create_or_update_user_link(discord_id: str, manager_id: str, discord_user: dict, zamboni_tag: str):
    ...
    existing = conn.execute('SELECT id FROM user_links WHERE discord_id = ?', (discord_id,)).fetchone()
...
def discord_oauth_callback():
    ...
    try:
        ...
        if not access_token:
            return redirect('/?portal=1&portalError=token')
        ...
        return redirect('/?portal=1&portalError=fetch')
